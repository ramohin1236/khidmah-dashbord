...
export default function AddClientModal({
    ...
    const handleSave = () => {
        if (!logoFile) {
            message.warning("Please choose a logo file");
            return;
        }
    ...
    return (
        ...
        >
            <div className={`bg-white rounded-2xl p-8 w-full max-w-md relative transform transition-all duration-300 ${isAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                ...
                <h2 className="text-3xl font-bold text-[#1E293B] mb-2 text-center">Add Client</h2>
                <p className="text-base text-[#64748B] mb-8 text-center">Add a new client logo and name.</p>

                <div className="space-y-6">
                    <div>
                        <label className="block text-base font-semibold text-[#1E293B] mb-2">Upload Logo</label>
                        <div className="flex items-center gap-4">
                            <label className="flex items-center gap-3 cursor-pointer px-4 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg">
                                ...
                                <div className="w-12 h-12 bg-white rounded-md overflow-hidden flex items-center justify-center border">
                                    {previewUrl ? (
                                        ...
                                        <span className="text-sm text-[#64748B]">No file</span>
                                    )}
                                </div>
                                <div>
                            ...
                            {previewUrl && (
                                <button type="button" onClick={() => { setLogoFile(null); setPreviewUrl(null); }} className="text-sm text-[#EF4444]">Remove</button>
                            )}
                        </div>
                    </div>
                </div>
